...
class highwayEnv():
    ...
    def __init__(self, render_mode):
        ...
        # Weather settings ------------------------------------------------------------------------
        weather = carla.WeatherParameters(sun_altitude_angle=90.0, sun_azimuth_angle=-90)
        ...
        # Set spectator ---------------------------------------------------------------------------
        spawn_point = self.map.get_waypoint_xodr(38,-2,40)
        spectator = self.map.get_waypoint_xodr(38,-3,40)
        world_transform = carla.Transform(carla.Location(x=spectator.transform.location.x-100, y=spectator.transform.location.y+10, z=65),
        ...
        # self.vehicles = [-1,-1,-2,-3,-3,-1,-2,-2,-1,-3,-2,-1,-2,-2,-3]
        self.vehicles = [-3, -2, -1]
        ...
        #     if (50 + i*16) < 216:
        #         wp = self.map.get_waypoint_xodr(38,self.vehicles[randint(0,2)],(50 + i*16))
        #     else:
        #         wp = self.map.get_waypoint_xodr(39,self.vehicles[self.vehicles_index],((i-i_aux)*16))
                
        #     t = carla.Transform(carla.Location(x=wp.transform.location.x, y=wp.transform.location.y, z=10), carla.Rotation(yaw=wp.transform.rotation.yaw))
        #     vehicle_bp = self.blueprint_library.find('vehicle.tesla.model3')
        #     adv = self.world.try_spawn_actor(vehicle_bp, t)
        #     adv.apply_control(carla.VehicleControl(throttle=0, steer=0, brake=1))
    ...
    def define_transforms(self):
        waypoint_1 = self.map.get_waypoint_xodr(38,-3,50)
        waypoint_2 = self.map.get_waypoint_xodr(38,-2,50)
        waypoint_3 = self.map.get_waypoint_xodr(38,-1,50)
        ...
        transform_3 = carla.Transform(carla.Location(x=waypoint_3.transform.location.x, y=waypoint_3.transform.location.y, z=1), carla.Rotation(yaw=waypoint_3.transform.rotation.yaw))
    ...
    def scenario(self):
        ...
        actors = self.world.get_actors().filter('vehicle.*.*')
        if len(actors) > 0 and self.path_published:
            self.path_published = False
            os.system("cd " + self.path + "&& ./route_lc.sh &")

        # if (time.time() - self.time_reset) > 6:
        #     if self.vehicles_index < (len(self.vehicles)-1):
        #         self.vehicles_index += 1
        #     else: 
        #         self.vehicles_index = 0
        ...
        #     self.time_reset = time.time()
        #     transform = self.transforms[self.vehicles[self.vehicles_index]]
        #     vehicle_bp = self.blueprint_library.find('vehicle.tesla.model3')
        ...
        #         self.tm.distance_to_leading_vehicle(adversary,10)
        #         self.tm.auto_lane_change(adversary, False)
        #         self.tm.vehicle_percentage_speed_difference(adversary,60)

        # for _, actor in enumerate(actors):
        #     if actor.id != self.ego_vehicle.id:
        #         actor.set_autopilot(True,self.tm_port)
        #         self.tm.ignore_lights_percentage(actor,100)
        #         self.tm.distance_to_leading_vehicle(actor,10)
        #         self.tm.auto_lane_change(actor, False)
        #         self.tm.vehicle_percentage_speed_difference(actor,60)
        ...
        #         if (waypoint.road_id == 39 and waypoint.s > 150):
        #            actor.destroy()
