...
class roundaboutEnv():
    ...
    def __init__(self, render_mode):
        ...
        # Set spectator ---------------------------------------------------------------------------
        spawn_point = self.map.get_waypoint_xodr(30,5,40)
        spectator = self.map.get_waypoint_xodr(30,5,5)
        world_transform = carla.Transform(carla.Location(x=spectator.transform.location.x, y=spectator.transform.location.y - 50, z=100),
    ...
    def define_transforms(self):
        waypoint_1 = self.map.get_waypoint_xodr(61,1,20)
        waypoint_2 = self.map.get_waypoint_xodr(61,1,2)
    ...
    def scenario(self):
        ...
        if len(actors) > 3 and self.path_published:
            self.path_published = False
            os.system("cd " + self.path + "&& ./route_ra.sh &")
        ...
        if (time.time() - self.time_reset) > self.vehicles[self.vehicles_index]:
            ...
            if adversary:
                ...
                self.tm.distance_to_leading_vehicle(adversary,randint(0,1))
                self.tm.auto_lane_change(adversary, False)
                self.tm.vehicle_percentage_speed_difference(adversary,randint(10,40))
        ...
        for _, actor in enumerate(actors):
            if actor.id != self.ego_vehicle.id:
                ...
                if waypoint.road_id == 1737 or waypoint.road_id == 46 or waypoint.road_id == 28 or (waypoint.road_id == 28 and waypoint.lane_id == -1):
                   actor.destroy()
