...
class mergeEnv():
    ...
    def __init__(self, render_mode):
        ...
        # Set spectator ---------------------------------------------------------------------------
        spawn_point = self.map.get_waypoint_xodr(29,-2,10)
        spectator = self.map.get_waypoint_xodr(29,-1,30)
        world_transform = carla.Transform(carla.Location(x=spectator.transform.location.x, y=spectator.transform.location.y, z=70),
    ...
    def define_transforms(self):
        waypoint_1 = self.map.get_waypoint_xodr(2,3,35)
        waypoint_2 = self.map.get_waypoint_xodr(2,2,35)
    ...
    def scenario(self):
        ...
        for _, actor in enumerate(actors):
            if actor.id != self.ego_vehicle.id:
                ...
                if (waypoint.road_id == 1 and waypoint.s < 5):
                   actor.destroy()
