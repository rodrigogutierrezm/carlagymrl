...
class benchmarkEnv():
    ...
    def __init__(self, render_mode):
        ...
        # Set spectator ---------------------------------------------------------------------------
        spawn_point = self.map.get_waypoint_xodr(30,5,70)
        spectator = self.map.get_waypoint_xodr(30,5,5)
        world_transform = carla.Transform(carla.Location(x=spectator.transform.location.x, y=spectator.transform.location.y + 30, z=70),
                                          carla.Rotation(yaw=270, pitch = -90))
        self.world.get_spectator().set_transform(world_transform)

        # Spawn ego vehicle -----------------------------------------------------------------------------
        self.blueprint_library = self.world.get_blueprint_library()
        self.ego_vehicle_transform = carla.Transform(carla.Location(x=spawn_point.transform.location.x, y=spawn_point.transform.location.y, z=0.6), 
        ...
        if actors:
            for actor in actors:
                if actor.type_id == 'vehicle.audi.a2':
                    ...
                    # print('\033[1;93m'+'Ego-vehicle found Scenario.' +'\033[0;m')
                    break
        else: 
            ...
            print('\033[1;93m'+'Ego-vehicle spawned Scenario.' +'\033[0;m')
        ...
        self.vehicles = [3,3,3,3,3,3,3,3,8]
    ...
    def define_transforms(self):
        waypoint_1 = self.map.get_waypoint_xodr(61,1,2)
       
        transform_1 = carla.Transform(carla.Location(x=waypoint_1.transform.location.x, y=waypoint_1.transform.location.y, z=0.6), carla.Rotation(yaw=waypoint_1.transform.rotation.yaw))
        waypoint_2 = self.map.get_waypoint_xodr(2,3,35) 
        transform_2 = carla.Transform(carla.Location(x=waypoint_2.transform.location.x, y=waypoint_2.transform.location.y, z=0.6), carla.Rotation(yaw=waypoint_2.transform.rotation.yaw))
        
        waypoint_left_1 = self.map.get_waypoint_xodr(19, 1, 20)
        transform_left_1 = carla.Transform(carla.Location(x=waypoint_left_1.transform.location.x, y=waypoint_left_1.transform.location.y, z=0.6), carla.Rotation(yaw=waypoint_left_1.transform.rotation.yaw))
        ...
        transforms = [transform_1, transform_2, transform_left_1, transform_right_1]        
    ...
    def scenario(self):
        ...
        # if len(actors) > 5 and self.path_published:
        #     self.path_published = False
        #     os.system("cd " + self.path + "&& ./route_concat.sh &")
        ...
        if (time.time() - self.time_reset) > self.vehicles[self.vehicles_index]:
            ...
            for i in range(len(self.transforms)):
                ...
                if adversary:
                    ...
                    self.tm.set_route(adversary, route)
        ...
        for _, actor in enumerate(actors):
            if actor.id != self.ego_vehicle.id:
                ...
                if waypoint.road_id == 1737 or \
                   waypoint.road_id == 46 or \
                   (waypoint.road_id == 28 and waypoint.s > 40) or \
                   (waypoint.road_id == 18 and waypoint.lane_id > 0) or \
                   (waypoint.road_id == 19 and waypoint.lane_id < 0) or \
                   (waypoint.road_id == 1 and waypoint.s < 5):
                   actor.destroy()
                elif waypoint.road_id == 28 and waypoint.s > 20:
                    self.tm.vehicle_percentage_speed_difference(actor,80)
